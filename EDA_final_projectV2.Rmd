...
In this project, I inversitgated over 150 river stations in europe, and issuses relating to changes in seasonal discharge, particularly after 1980


## analysis of stations

### station location
stations are located in europe, particularly density of location in mainland western europe and scandinavia, there are some stations sparely distributed in Russia and eastern europe, and the British Isles and Iceland.
```{r, echo = FALSE}
...
There is a large cluster of stations around 45-50 degrees in latitude and 5 to 15 degrees in longitude, theses would correspond to rivers mainly flowing out of alpine regions in Switzerland and Germany, with higher altitudes, as latitude increases, the altitude decreases, this is typical of  Germany, since the coastline is in the north.
...
### distribution by nmber of years of recorded data
while this is from info given, actual dataset may differ
...
calculating the mean of the ratios of max/mean and min/mean, for the period befor 1980 and after 1980, then calculating the difference between values shows changes in runoff post 1980 relative to pre 1980
```{r, echo=FALSE}
...
### percentage change
plotting reulsts, in winter, runnoff has increased over the period 1980-present, this relationship is unclear in the summer period with different rivers showing falls and rises, firther investigations would be needed to indentify why this result exists
...
LUETSCHINE - a small river heading to a lkae, in high altitude influenced heavily by snowmelt
glama – many power stations have been built along the river.
...
3 of the rivers, on the rhine, the sitter and the danube, show flat profiles over months, river glama is probably heavly influeced by snow melt due to large rises in runnoff in spring and summer months, as is the Luetshine river which, is a short river at high altitude
```{r, echo=FALSE,  warning = FALSE, message = FALSE}
...
winter runoff is increasing as a function of time- espcially at river glama, in summer runnoff, runnoff is faling except for station luechine
```{r, echo=FALSE}
